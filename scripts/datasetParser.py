...
class DatasetParser:
    ...
    def is_in_bbox_readable(self, bbox, point):
    ...
    def is_in_bbox_fast(self, bbox, point):
        """Return True if point within bbox in xy-plane.
        ...
        # Check that point is between both sets of parallel lines
        return True if self.is_between_lines_readable(w1, w2, point) \
            and self.is_between_lines_readable(l1, l2, point) else False
    ...
    def is_between_lines_fast(self, l1, l2, px, py):
        """Return true if point is between parallel lines.

        Args:
            l1: function that returns y for any x
            l2: function that returns y for any x
            px: x-coord of point in question
            py: y-coord ...
        ...
        # If line 1 is above line 2
        if y1 > y2:
            return True if y2 < py < y1 else False
        else:
            return True if y1 < py < y2 else False
...
if __name__ == "__main__":
    ...
    for data in dataset:
        ...
        pcls = parser.compute_clusters(pcl, bboxes)
